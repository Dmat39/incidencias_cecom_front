  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Serenos</h1>
          <p className="text-sm text-gray-500">Gestión del personal de serenazgo</p>
        </div>
              ...
              <TableBody>
                {isLoading ? (
                  ...
                  serenos.map((s: Sereno, idx: number) => (
                    ...
                    >
                      <TableCell className="py-2.5 font-mono text-sm text-gray-700">{s.dni || '-'}</TableCell>
                      <TableCell className="py-2.5 text-sm text-gray-800">
                        {[s.apellidoPaterno, s.apellidoMaterno, s.nombres].filter(Boolean).join(' ') || '-'}
                      </TableCell>
                      <TableCell className="py-2.5 text-sm text-gray-600">{s.cargoSereno?.descripcion || '-'}</TableCell>
                      <TableCell className="py-2.5">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${s.habilitado ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      ...
                      <TableCell className="py-2.5">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            className={`inline-flex items-center justify-center w-8 h-8 rounded transition-colors ${s.habilitado ? 'text-yellow-600 hover:bg-yellow-50' : 'text-green-600 hover:bg-green-50'}`}
                          >
