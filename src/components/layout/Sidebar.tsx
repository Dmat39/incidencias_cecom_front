  ...
  return (
    <aside className="w-64 min-h-screen bg-white border-r border-gray-200 flex flex-col">
      ...
      <div className="px-4 py-5 border-b border-gray-200 flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-gray-800 leading-tight">CECOM</p>
          <p className="text-xs text-gray-500 leading-tight">Sistema de Incidencias</p>
        </div>
      </div>
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        ...
        {modulosPermitidos.includes('svi') && <div>
            ...
            >
              <Shield className={cn('h-5 w-5', sviActive ? 'text-slate-600' : 'text-gray-400')} />
              <span className="flex-1 text-left">SVI</span>
              {sviAuth && (
                <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-semibold mr-1">
                  activo
                </span>
              )}
              <ChevronDown
                className={cn('h-4 w-4 text-gray-400 transition-transform', sviOpen && 'rotate-180')}
              />
            </button>
              ...
              <div className="ml-4 mt-1 space-y-1 border-l border-gray-200 pl-3">
                {sviSubItems.map((sub) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={cn('h-4 w-4', isActive ? 'text-slate-600' : 'text-gray-400')} />
                      {sub.label}
                    </Link>
                  );
                })}
                ...
                {sviAuth && (
                  <button
                    onClick={() => sviLogout()}
                    className="w-full flex items-center gap-2 px-2 py-2 text-xs text-red-500 hover:bg-red-50 rounded-md transition-colors"
                  >
      ...
      <div className="px-4 py-4 border-t border-gray-200">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-800 truncate">{user?.username || 'Usuario'}</p>
